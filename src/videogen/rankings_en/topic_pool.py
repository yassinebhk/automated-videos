...
Reusa el generador de `ranking` (matplotlib chart race + música Pixabay) vía
`RankingBranding(lang="en", ...)`. Contenido NUEVO en inglés (NO traducción del
pool ES): rankings globales/internacionales evergreen con fuente pública citable.

Campos (mismo schema que ranking ES para reusar el generador):
- key, audiencia, categoria, titulo (EN), fuente (fuente pública), dataset_hint
  (pista para que el LLM genere datos REALES), cifra_ancla (unidad/dato clave).

Veracidad: cada topic cita una fuente pública verificable; el generador pide al
LLM datos coherentes con esa fuente y marca la descripción como aproximada.
YMYL excluido (sin salud/medicina/política/cripto/inversión).
Rotación: ledger output/rankings_en_ledger.json, cooldown 90 días.
"""
...
TOPICS = [
    ...
    {"key": "richest_people", "audiencia": "general", "categoria": "economy",
     ...
     "dataset_hint": "Gates 2000s → Bezos/Musk/Arnault 2020s, yearly net worth",
    ...
    {"key": "smartphone_brands", "audiencia": "general", "categoria": "tech",
     ...
     "cifra_ancla": "% global market share"},
    ...
    {"key": "tallest_buildings", "audiencia": "general", "categoria": "engineering",
     ...
     "fuente": "CTBUH (Council on Tall Buildings)",
    ...
    {"key": "car_manufacturers_sales", "audiencia": "general", "categoria": "business",
     ...
     "cifra_ancla": "million vehicles/year"},
    ...
    {"key": "most_streamed_spotify", "audiencia": "general", "categoria": "music",
     ...
     "dataset_hint": "Bad Bunny, Taylor Swift, Drake, The Weeknd — monthly listeners",
